let workshopShowcase = {
    ...
    downloadImages: function () {
        ...
        if (inputImage.file.type != 'image/gif') {
            ...
            let cropCanvas = new CustomCanvas(workshopShowcase.sliceWidth, imgHeight);
            ws_cropImages(
...
function ws_cropImages(zip, canvasEl, sliceNum) { // Recursive function for cropping images
    ...
    // if ($('#resizeImage').is(':checked')) {
    //     let temp = new CustomCanvas(626, getComputedValueFor(workshopShowcase.imgPreview[0], 'height'));
    //     temp.addCanvas()
...
function ws_cropGifs(zip, gifs, currentSlice) {
    // Recursive function for cropping gifs
    let patcher = new FileReader();
    patcher.onload = function () {
        let binaryImage = patcher.result;
        for (let i = binaryImage.length - 1; i != 0; i--) { // Trim empty bytes and add 21 at the end
        ...
        if (currentSlice != 5) {
            ws_cropGifs(zip, gifs, currentSlice + 1);
        } else {
            inputImage.setStatusMsg("Creating zip file, please wait...");
            zip.generateAsync({
                type: "blob"
    ...
    for (let i = 0; i < gifs.length; i++) {
        ...
        let frame = new CustomCanvas(workshopShowcase.sliceWidth, imgHeight);
...
function ws_getSliceOffsetSmall(sliceNum) {
    switch (sliceNum) {
        case 1:
            return 0;
        case 2:
